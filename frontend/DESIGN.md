...
> 签名瞬间：登录页为**科幻深空场景**（本地 SVG 静态资产 + 组件内动效叠加层：星野/星云/行星弧/透视网格 + 星云慢漂移/星闪/流星循环/地平线辉光脉动，慢节奏、仅 transform/opacity，见 `docs/adr/0001-login-ambient-motion.md`），登录卡在此场景下采用**深色玻璃变体**（深蓝半透明 + 白字 + 暗玻璃输入框 + CTA 蓝色辉光 + hover sheen 扫过），与后台浅色玻璃形成"入口→工作区"的明暗对比；后台顶栏为悬浮玻璃层，页面内容从其下方模糊穿过。
...
- 背景光斑**不动画**（登录页深空动效层为唯一例外，见 `docs/adr/0001-login-ambient-motion.md`）；backdrop-filter 只用于固定层（顶栏/侧栏/弹层）与少量静态卡片，禁止用于滚动中的大内容区
...
- 登录页例外：科幻深空场景固定不随主题切换（深空本身即"夜"），登录卡恒为深色玻璃；场景带一层组件内联的 CSS 环境动效（星云漂移/星闪/流星/辉光脉动），同样不随主题翻转
